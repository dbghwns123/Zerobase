...
//  배열에 데이터 삽입
    public void insertData(int index, int data) {
        ...
        int[] arrDup = this.arr.clone();      //arrDup에 원래 arr을 클론
        this.arr = new int[this.arr.length + 1];

        for (int i = 0; i < index; i++) {      //index값 전까지 데이터 삽입
            this.arr[i] = arrDup[i];
        }
        this.arr[index] = data;                //indax값 그대로 data 삽임

        for (int i = index + 1; i < this.arr.length; i++) {    //index값 뒤에 데이터 삽입
